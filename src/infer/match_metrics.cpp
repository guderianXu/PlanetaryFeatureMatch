...
std::pair<int64_t, int64_t> map_feature_point_to_image(
    ...
    const int64_t x = std::min<int64_t>(
        image_width - 1,
        std::max<int64_t>(0, std::llround((point.index({0}).item<float>() + 0.5F) * scale_x - 0.5F)));
    const int64_t y = std::min<int64_t>(
        image_height - 1,
        std::max<int64_t>(0, std::llround((point.index({1}).item<float>() + 0.5F) * scale_y - 0.5F)));
    return {x, y};
}

std::pair<double, double> map_feature_point_to_image_float(
    const torch::Tensor& point,
    int64_t map_width,
    int64_t map_height,
    int64_t image_width,
    ...
    return {
        std::min<double>(image_width - 1, std::max<double>(0.0, x)),
        std::min<double>(image_height - 1, std::max<double>(0.0, y))};
}

bool is_correct_pixel_match(
...
WarpMatchMetrics compute_warp_match_metrics(
    ...
    if (matches.sparse_matches.defined() && matches.sparse_matches.numel() > 0 && features_a.keypoints.defined() &&
        ...
        for (int64_t index = 0; index < sparse_matches.size(0); ++index) {
            ...
            const auto [x_a, y_a] = map_feature_point_to_image(
                points_a[index_a],
                features_a.feature_map_width,
                features_a.feature_map_height,
                warp.size(1),
                warp.size(0));
            const auto [x_b, y_b] = map_feature_point_to_image(
                points_b[index_b],
...
WarpFeatureCoverageMetrics compute_warp_feature_coverage_metrics(
    const FeatureSet& features_a,
    const FeatureSet& features_b,
    const torch::Tensor& warp_a_to_b,
    double correct_threshold_pixels
    ...
    if (!features_a.keypoints.defined() || !features_b.keypoints.defined() ||
        !features_a.descriptors.defined() || !features_b.descriptors.defined()) {
        throw std::invalid_argument("feature keypoints and descriptors must be defined");
    }
    if (features_a.keypoints.dim() != 2 || features_b.keypoints.dim() != 2 ||
        features_a.keypoints.size(1) != 2 || features_b.keypoints.size(1) != 2 ||
    ...
    for (int64_t index_b = 0; index_b < points_b.size(0); ++index_b) {
        image_points_b.push_back(map_feature_point_to_image_float(
            points_b[index_b],
            features_b.feature_map_width,
            features_b.feature_map_height,
            warp.size(1),
    ...
    for (int64_t index_a = 0; index_a < points_a.size(0); ++index_a) {
        ...
        if (!std::isfinite(expected_x) || !std::isfinite(expected_y) ||
            expected_x < 0.0F || expected_x > static_cast<float>(warp.size(1) - 1) ||
            expected_y < 0.0F || expected_y > static_cast<float>(warp.size(0) - 1)) {
            continue;
        }
        ...
        for (const auto positive_index : positive_indices) {
            best_positive_score = std::max(
                best_positive_score,
                descriptor_cosine(descriptors_a[index_a], descriptors_b[positive_index]));
        }
        int64_t rank = 1;
    ...
    if (metrics.valid_warp_total > 0) {
        metrics.coverage_fraction = static_cast<double>(metrics.covered_by_target_keypoint) /
