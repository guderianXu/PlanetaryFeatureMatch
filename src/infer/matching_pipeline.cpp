...
bool shouldPreferMutualDescriptorGeometry(int64_t mutual_matches, int64_t topk_matches) {
    ...
    if (topk_matches <= DESCRIPTOR_MUTUAL_GEOMETRY_CLOSE_TOPK_MAX_MATCHES &&
        topk_matches >= mutual_matches &&
...
std::pair<torch::Tensor, torch::Tensor> matchDescriptorTopKFeatures(
    const FeatureSet& features_a,
    const FeatureSet& features_b,
    ...
    auto source_indices = torch::arange(scores.size(0), long_options)
                              .unsqueeze(1)
                              .expand({scores.size(0), k})
                              .reshape({-1})
                              .contiguous();
...
std::pair<torch::Tensor, torch::Tensor> matchDescriptorReciprocalTopKFeatures(
    const FeatureSet& features_a,
    const FeatureSet& features_b,
    int64_t candidates_per_source = DESCRIPTOR_TOPK_CANDIDATES_PER_SOURCE,
    const torch::Device& compute_device = torch::Device(torch::kCPU)
    ...
    return {
        torch::from_blob(
            match_values.data(),
            {static_cast<int64_t>(score_values.size()), 2},
            long_options)
            .clone()
            .contiguous(),
        torch::from_blob(
            score_values.data(),
            {static_cast<int64_t>(score_values.size())},
            float_options)
            .clone()
            .contiguous()};
}

std::pair<torch::Tensor, torch::Tensor> mergeSparseMatchCandidates(
    ...
    return {
        torch::from_blob(
            merged_matches.data(),
            {static_cast<int64_t>(merged_scores.size()), 2},
            long_options)
            .clone()
            .contiguous(),
        torch::from_blob(
            merged_scores.data(),
            {static_cast<int64_t>(merged_scores.size())},
            float_options)
            .clone()
            .contiguous()};
}

std::pair<torch::Tensor, torch::Tensor> selectMatches(
    const torch::Tensor& matches,
    const torch::Tensor& scores,
    ...
    auto keep = torch::from_blob(
                    const_cast<int64_t*>(keep_indices.data()),
                    {static_cast<int64_t>(keep_indices.size())},
                    long_options)
                    .clone();
    return {
        matches.to(torch::kCPU, torch::kInt64).index_select(0, keep).contiguous(),
        scores.to(torch::kCPU, torch::kFloat32).index_select(0, keep).contiguous()};
}

std::pair<torch::Tensor, torch::Tensor> trimLowConfidenceTopKTail(
    const torch::Tensor& matches,
    const torch::Tensor& scores
) {
    if (!matches.defined() || !scores.defined() ||
        matches.size(0) < DESCRIPTOR_TOPK_TAIL_TRIM_MIN_MATCHES ||
...
std::pair<torch::Tensor, torch::Tensor> cleanupAffineResidualMatches(
    const FeatureSet& features_a,
    const FeatureSet& features_b,
    const torch::Tensor& matches,
    const torch::Tensor& scores
    ...
    const auto cleanup_threshold = low_count
        ? GEOMETRIC_RESIDUAL_CLEANUP_LOW_COUNT_THRESHOLD
        : GEOMETRIC_RESIDUAL_CLEANUP_THRESHOLD;
    for (int64_t row = 0; row < cpu_matches.size(0); ++row) {
        ...
        source_points.emplace_back(
            points_a.index({source, 0}).item<float>(),
            points_a.index({source, 1}).item<float>());
        target_points.emplace_back(
            points_b.index({target, 0}).item<float>(),
    ...
    auto affine_keep = [&]() {
        ...
        cv::Mat mask;
        const auto affine = cv::estimateAffine2D(
            source_points,
            target_points,
            mask,
            cv::RANSAC,
        ...
        for (int64_t row = 0; row < cpu_matches.size(0); ++row) {
            ...
            const double predicted_x =
                affine.at<double>(0, 0) * source.x +
                affine.at<double>(0, 1) * source.y +
                affine.at<double>(0, 2);
            const double predicted_y =
                affine.at<double>(1, 0) * source.x +
                affine.at<double>(1, 1) * source.y +
                affine.at<double>(1, 2);
            const double residual = std::hypot(
                predicted_x - static_cast<double>(target.x),
                predicted_y - static_cast<double>(target.y));
    ...
    auto homography_keep = [&]() {
        ...
        for (int64_t row = 0; row < cpu_matches.size(0); ++row) {
            ...
            const auto& target = target_points[static_cast<std::size_t>(row)];
            const double denom =
                homography.at<double>(2, 0) * source.x +
                homography.at<double>(2, 1) * source.y +
                homography.at<double>(2, 2);
            if (std::abs(denom) <= 1.0e-12) {
                continue;
            }
            const double predicted_x =
                (homography.at<double>(0, 0) * source.x +
                 homography.at<double>(0, 1) * source.y +
                 homography.at<double>(0, 2)) /
                denom;
            const double predicted_y =
                (homography.at<double>(1, 0) * source.x +
                 homography.at<double>(1, 1) * source.y +
    ...
    const auto min_keep = low_count
        ? std::max<int64_t>(GEOMETRIC_RESIDUAL_CLEANUP_LOW_COUNT_MIN_KEEP, cpu_matches.size(0) / 2)
...
std::vector<int64_t> remapMaskIndices(
...
double pointBoundingArea(
    ...
    return static_cast<double>(std::max(0.0F, max_x - min_x)) *
           static_cast<double>(std::max(0.0F, max_y - min_y));
}

double pointSpreadRatio(
    const std::vector<cv::Point2f>& points,
    const std::vector<int64_t>& indices,
...
GeometricCandidate estimateBestGeometricCandidate(
    ...
    cv::estimateAffine2D(
        ...
        affine_mask,
        cv::RANSAC,
        GEOMETRIC_CONSISTENCY_RANSAC_THRESHOLD);
...
std::pair<torch::Tensor, torch::Tensor> filterProjectiveConsistentMatches(
    const FeatureSet& features_a,
    const FeatureSet& features_b,
    const torch::Tensor& matches,
    const torch::Tensor& scores
    ...
    for (const auto prefix_size : geometricConsistencyPrefixSizes(static_cast<int64_t>(sorted_indices.size()))) {
        auto candidate = estimateBestGeometricCandidate(
            source_points,
            target_points,
            score_values,
    ...
    std::sort(best.indices.begin(), best.indices.end(), [&](int64_t lhs, int64_t rhs) {
        return score_values[static_cast<std::size_t>(lhs)] >
               score_values[static_cast<std::size_t>(rhs)];
    });
...
std::pair<torch::Tensor, torch::Tensor> mutualGraphLogitMatches(
    ...
    auto pair_logits = logits.index({
        torch::indexing::Slice(0, keypoint_count_a),
        torch::indexing::Slice(0, keypoint_count_b)}).to(torch::kCPU, torch::kFloat32).contiguous();
    auto row_logits = logits.index({
        torch::indexing::Slice(0, keypoint_count_a),
        torch::indexing::Slice(0, keypoint_count_b + 1)}).to(torch::kCPU, torch::kFloat32).contiguous();
    auto col_logits = logits.index({
        torch::indexing::Slice(0, keypoint_count_a + 1),
        torch::indexing::Slice(0, keypoint_count_b)}).to(torch::kCPU, torch::kFloat32).contiguous();
    ...
    for (int64_t source = 0; source < keypoint_count_a; ++source) {
        ...
        if (target < keypoint_count_b && best_ba.index({target}).item<int64_t>() == source) {
            candidates.push_back(Candidate{
                source,
                target,
                row_probs.index({source, target}).item<float>()});
        }
    }
...
std::pair<torch::Tensor, torch::Tensor> greedyGraphLogitMatches(
    const torch::Tensor& logits,
    int64_t keypoint_count_a,
    int64_t keypoint_count_b
) {
    ...
    auto pair_logits = logits.index({
        torch::indexing::Slice(0, keypoint_count_a),
        torch::indexing::Slice(0, keypoint_count_b)}).to(torch::kCPU, torch::kFloat32).contiguous();
    auto row_dustbin = logits.index({
        torch::indexing::Slice(0, keypoint_count_a),
        keypoint_count_b}).to(torch::kCPU, torch::kFloat32).contiguous();
    auto col_dustbin = logits.index({
        keypoint_count_a,
        torch::indexing::Slice(0, keypoint_count_b)}).to(torch::kCPU, torch::kFloat32).contiguous();
    auto row_logits = logits.index({
        torch::indexing::Slice(0, keypoint_count_a),
    ...
    std::vector<int64_t> matches;
    std::vector<float> scores;
    const auto max_matches = std::min<int64_t>(
        GRAPH_GREEDY_MAX_MATCHES,
        std::min(keypoint_count_a, keypoint_count_b));
    ...
    for (int64_t rank = 0; rank < sorted_indices.size(0) && static_cast<int64_t>(scores.size()) < max_matches; ++rank) {
        ...
        if (score <= row_dustbin.index({source}).item<float>() ||
            score <= col_dustbin.index({target}).item<float>()) {
            continue;
        }
...
std::pair<torch::Tensor, torch::Tensor> relaxedGraphLogitMatches(
    const torch::Tensor& logits,
    int64_t keypoint_count_a,
    int64_t keypoint_count_b
) {
...
std::pair<torch::Tensor, torch::Tensor> matchSparseFeatures(
    ...
    std::pair<torch::Tensor, torch::Tensor> greedy{
    ...
    if (shouldUseGraphMatcherForSparseCount(features_a.descriptors.size(0), features_b.descriptors.size(0))) {
        ...
        greedy = mutualGraphLogitMatches(
            output.logits,
            features_a.descriptors.size(0),
            features_b.descriptors.size(0));
        if (greedy.first.size(0) < GEOMETRIC_CONSISTENCY_MIN_MATCHES) {
            greedy = greedyGraphLogitMatches(
                output.logits,
                features_a.descriptors.size(0),
                features_b.descriptors.size(0));
        }
        if (greedy.first.size(0) < GEOMETRIC_CONSISTENCY_MIN_MATCHES) {
            greedy = relaxedGraphLogitMatches(
                output.logits,
                features_a.descriptors.size(0),
                features_b.descriptors.size(0));
        }
    } else if (debug) {
        std::cerr << "match debug: graph matcher skipped for sparse counts "
                  << features_a.descriptors.size(0) << "x" << features_b.descriptors.size(0)
                  << '\n';
    }
    ...
    if (debug) {
        std::cerr << "match debug: features_a=" << features_a.descriptors.size(0)
                  ...
                  << " descriptor_matches=" << descriptor_matches.first.size(0)
                  << " graph_matches=" << matches.size(0) << '\n';
    }
    if (returnRawGraphMatchesForDebug()) {
        return {matches, scores};
    }
    Timer debug_timer;
    auto descriptor_topk = useTopKGeometryForDebug()
        ? matchDescriptorTopKFeatures(
              features_a,
              features_b,
              descriptorTopKCandidatesPerSource(),
              matcher_device)
        : std::pair<torch::Tensor, torch::Tensor>{
              torch::empty({0, 2}, long_options),
              torch::empty({0}, float_options)};
    if (debug) {
        std::cerr << "match debug timing: descriptor_topk="
                  << formatSeconds(debug_timer.elapsedSeconds()) << "s\n";
        debug_timer.reset();
    }
    if (descriptor_topk.first.size(0) >= GEOMETRIC_CONSISTENCY_MIN_MATCHES) {
        if (descriptorTopKProjectiveBeforeRotation()) {
            ...
            if (debug) {
                std::cerr << "match debug timing: topk_projective_raw="
                          << formatSeconds(debug_timer.elapsedSeconds()) << "s\n";
                debug_timer.reset();
                std::cerr << "match debug: descriptor_topk_projective_raw_matches="
                          << topk_projective.first.size(0) << '\n';
            }
            if (topk_projective.first.size(0) >= GEOMETRIC_CONSISTENCY_MIN_INLIERS) {
                return topk_projective;
            }
        }
        auto topk_rotation = filterRotationConsistentMatches(
            features_a,
            features_b,
            descriptor_topk.first,
            descriptor_topk.second);
        auto unique_rotation = mergeSparseMatchCandidates(
            topk_rotation.first,
            topk_rotation.second,
            torch::empty({0, 2}, long_options),
            torch::empty({0}, float_options));
        if (debug) {
            std::cerr << "match debug timing: topk_rotation="
                      << formatSeconds(debug_timer.elapsedSeconds()) << "s\n";
            debug_timer.reset();
        }
        if (debug) {
        ...
        if (unique_rotation.first.size(0) >= GEOMETRIC_CONSISTENCY_MIN_MATCHES) {
            ...
            if (debug) {
                std::cerr << "match debug timing: topk_projective="
                          << formatSeconds(debug_timer.elapsedSeconds()) << "s\n";
                debug_timer.reset();
            }
            if (debug) {
                std::cerr << "match debug: descriptor_topk_geometric_matches="
            ...
            if (topk_consistent.first.size(0) >= GEOMETRIC_CONSISTENCY_MIN_INLIERS) {
                if (descriptor_matches.first.size(0) >= GEOMETRIC_CONSISTENCY_MIN_MATCHES) {
                    ...
                    if (descriptor_consistent.first.size(0) >= GEOMETRIC_CONSISTENCY_MIN_INLIERS &&
                        shouldPreferMutualDescriptorGeometry(
                            descriptor_consistent.first.size(0),
                            topk_consistent.first.size(0))) {
                ...
                if (topk_consistent.first.size(0) <= DESCRIPTOR_WIDE_TOPK_FALLBACK_MAX_BASE_MATCHES) {
                    auto wide_topk = matchDescriptorTopKFeatures(
                        features_a,
                        features_b,
                        DESCRIPTOR_WIDE_TOPK_CANDIDATES_PER_SOURCE,
                        matcher_device);
                    auto wide_rotation = filterRotationConsistentMatches(
                        features_a,
                        features_b,
                        wide_topk.first,
                        wide_topk.second);
                    auto unique_wide_rotation = mergeSparseMatchCandidates(
                        wide_rotation.first,
                        wide_rotation.second,
                        torch::empty({0, 2}, long_options),
                        torch::empty({0}, float_options));
        ...
        if (descriptorReciprocalTopKFallback()) {
            auto reciprocal_topk = matchDescriptorReciprocalTopKFeatures(
                ...
                descriptorTopKCandidatesPerSource(),
                matcher_device);
            auto reciprocal_rotation = filterRotationConsistentMatches(
                features_a,
                features_b,
                reciprocal_topk.first,
                reciprocal_topk.second);
            auto unique_reciprocal_rotation = mergeSparseMatchCandidates(
                reciprocal_rotation.first,
                reciprocal_rotation.second,
                torch::empty({0, 2}, long_options),
                torch::empty({0}, float_options));
            if (unique_reciprocal_rotation.first.size(0) >= GEOMETRIC_CONSISTENCY_MIN_MATCHES) {
                auto reciprocal_consistent = filterProjectiveConsistentMatches(
                    ...
                    unique_reciprocal_rotation.second);
                if (debug) {
    ...
    if (descriptor_matches.first.size(0) >= GEOMETRIC_CONSISTENCY_MIN_MATCHES) {
        auto descriptor_consistent = filterProjectiveConsistentMatches(
            ...
            descriptor_matches.second);
        if (debug) {
            std::cerr << "match debug: descriptor_geometric_matches="
                      << descriptor_consistent.first.size(0) << '\n';
        }
        if (descriptor_consistent.first.size(0) >= GEOMETRIC_CONSISTENCY_MIN_INLIERS) {
            return descriptor_consistent;
...
std::pair<torch::Tensor, torch::Tensor> descriptor_reciprocal_topk_matches_for_test(
    ...
    return matchDescriptorReciprocalTopKFeatures(
        features_a,
        features_b,
