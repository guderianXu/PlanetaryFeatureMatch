...
bool isSuppressedBySelected(const std::vector<SparseCandidate>& selected, const SparseCandidate& candidate, int radius)
{
                                               ...
                                               int nms_radius)
{
    ...
    std::sort(candidates.begin(), candidates.end(),
              [](const SparseCandidate& lhs, const SparseCandidate& rhs)
              {
...
bool candidateInCell(const SparseCandidate& candidate, int row, int col, const FeatureDecodeConfig& config,
...
bool containsCandidate(const std::vector<SparseCandidate>& candidates, const SparseCandidate& candidate)
{
    return std::any_of(candidates.begin(), candidates.end(),
                       [&](const SparseCandidate& selected)
                       {
...
void appendCandidateUntilLimit(std::vector<SparseCandidate>& selected, const SparseCandidate& candidate,
                               int max_keypoints)
{
...
std::vector<SparseCandidate> selectGridBalancedCandidates(const std::vector<SparseCandidate>& candidates,
...
void appendRawDescriptor(std::vector<float>& output, const float* descriptor_data, int64_t descriptor_channels,
                         int64_t height, int64_t width, int64_t y, int64_t x)
{
...
void addBilinearDescriptorSample(std::vector<float>& accumulator, float& weight_sum, const float* descriptor_data,
                                 const bool* mask_data, int64_t descriptor_channels, int64_t height, int64_t width,
                                 float sample_y, float sample_x, float sample_weight)
{
    if (!std::isfinite(sample_y) || !std::isfinite(sample_x) || sample_weight <= 0.0F || sample_y < 0.0F ||
        sample_x < 0.0F || sample_y > static_cast<float>(height - 1) || sample_x > static_cast<float>(width - 1))
    {
        return;
    }
...
void appendOrientationPooledDescriptor(std::vector<float>& output, const float* descriptor_data,
                                       const float* orientation_data, const bool* mask_data,
                                       ...
                                       int descriptor_pool_radius)
{
    ...
    const auto spatial_offset = y * width + x;
    addBilinearDescriptorSample(accumulator, weight_sum, descriptor_data, mask_data, descriptor_channels, height, width,
                                static_cast<float>(y), static_cast<float>(x), DESCRIPTOR_POOL_CENTER_WEIGHT);
    ...
    for (int radius = 1; radius <= descriptor_pool_radius; ++radius)
    {
        ...
        addBilinearDescriptorSample(accumulator, weight_sum, descriptor_data, mask_data, descriptor_channels, height,
                                    width, static_cast<float>(y) + axis_y * step, static_cast<float>(x) + axis_x * step,
                                    axis_weight);
        addBilinearDescriptorSample(accumulator, weight_sum, descriptor_data, mask_data, descriptor_channels, height,
                                    width, static_cast<float>(y) - axis_y * step, static_cast<float>(x) - axis_x * step,
                                    axis_weight);
        addBilinearDescriptorSample(accumulator, weight_sum, descriptor_data, mask_data, descriptor_channels, height,
                                    width, static_cast<float>(y) + ortho_y * step,
                                    static_cast<float>(x) + ortho_x * step, axis_weight);
        addBilinearDescriptorSample(accumulator, weight_sum, descriptor_data, mask_data, descriptor_channels, height,
                                    width, static_cast<float>(y) - ortho_y * step,
                                    static_cast<float>(x) - ortho_x * step, axis_weight);
        if (descriptor_pool_radius > 1)
        {
            ...
            addBilinearDescriptorSample(accumulator, weight_sum, descriptor_data, mask_data, descriptor_channels,
                                        height, width, static_cast<float>(y) + (axis_y + ortho_y) * step,
                                        static_cast<float>(x) + (axis_x + ortho_x) * step, diagonal_weight);
            addBilinearDescriptorSample(accumulator, weight_sum, descriptor_data, mask_data, descriptor_channels,
                                        height, width, static_cast<float>(y) + (axis_y - ortho_y) * step,
                                        static_cast<float>(x) + (axis_x - ortho_x) * step, diagonal_weight);
            addBilinearDescriptorSample(accumulator, weight_sum, descriptor_data, mask_data, descriptor_channels,
...
torch::Tensor prepare_decode_mask(const torch::Tensor& mask, int64_t height, int64_t width)
{
    ...
    {
        float_mask = torch::nn::functional::interpolate(float_mask, torch::nn::functional::InterpolateFuncOptions()
                               ...
                               const torch::Tensor& intensity_mask)
{
    ...
    const auto selected_candidates =
        selectGridBalancedCandidates(nms_candidates, unfiltered_candidates, config, height, width);
    ...
    const auto dense_points_tensor =
        dense_count == 0 ? torch::empty({0, 2}, tensor_options)
                         : torch::from_blob(dense_points.data(), {dense_count, 2}, tensor_options).clone().contiguous();
    const auto dense_confidence_tensor =
        dense_count == 0
            ? torch::empty({0}, tensor_options)
            : torch::from_blob(dense_confidence.data(), {dense_count}, tensor_options).clone().contiguous();

    const auto sparse_points_tensor =
        torch::from_blob(sparse_points.data(), {sparse_count, 2}, tensor_options).clone().contiguous();
    const auto sparse_scores_tensor =
        torch::from_blob(sparse_scores.data(), {sparse_count}, tensor_options).clone().contiguous();
    const auto sparse_descriptors_tensor =
        torch::from_blob(sparse_descriptors.data(), {sparse_count, descriptors.size(1)}, tensor_options)
            .clone()
            .contiguous();
    const auto sparse_scale_tensor =
        torch::from_blob(sparse_scale.data(), {sparse_count}, tensor_options).clone().contiguous();
    const auto sparse_orientation_tensor =
        torch::from_blob(sparse_orientation.data(), {sparse_count, orientation.size(1)}, tensor_options)
            .clone()
            .contiguous();
    const auto sparse_affine_tensor =
        torch::from_blob(sparse_affine.data(), {sparse_count, 2, 2}, tensor_options).clone().contiguous();

    return FeatureSet{sparse_points_tensor, sparse_scores_tensor,      sparse_descriptors_tensor,
                      sparse_scale_tensor,  sparse_orientation_tensor, sparse_affine_tensor,
                      dense_points_tensor,  dense_confidence_tensor};
}
