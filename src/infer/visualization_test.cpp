...
pfm::FeatureSet make_feature_set(torch::Tensor keypoints)
{
    ...
    return pfm::FeatureSet{keypoints.to(torch::kFloat32).contiguous(), torch::ones({count}, torch::kFloat32),
                           torch::zeros({count, 8}, torch::kFloat32),  torch::ones({count}, torch::kFloat32),
                           torch::zeros({count}, torch::kFloat32),     torch::zeros({count, 2, 2}, torch::kFloat32),
...
pfm::MatchSet make_match_set(torch::Tensor points_a, torch::Tensor points_b, torch::Tensor confidence)
{
    ...
    return pfm::MatchSet{torch::stack({torch::arange(count, torch::kLong), torch::arange(count, torch::kLong)}, 1),
                         confidence.to(torch::kFloat32).contiguous(), points_a.to(torch::kFloat32).contiguous(),
                         points_b.to(torch::kFloat32).contiguous(), confidence.to(torch::kFloat32).contiguous()};
}
...
static void visualization_writes_feature_keypoint_png()
{
    ...
    const auto output_path =
        pfm::save_feature_visualization(image_path.string(), features, temp_dir.file("vis").string());
...
static void visualization_scales_feature_map_keypoints_to_image_pixels()
{
    ...
    const auto output_path =
        pfm::save_feature_visualization(image_path.string(), features, temp_dir.file("vis").string(), 10, 8);
...
static void visualization_writes_match_png()
{
    ...
    const auto matches = make_match_set(torch::tensor({{4.0F, 5.0F}, {18.0F, 10.0F}}, torch::kFloat32),
                                        torch::tensor({{6.0F, 5.0F}, {20.0F, 12.0F}}, torch::kFloat32),
                                        torch::tensor({0.4F, 0.9F}, torch::kFloat32));

    const auto output_path = pfm::save_match_visualization(image_a_path.string(), image_b_path.string(), matches,
...
static void visualization_writes_match_png_without_matches()
{
    ...
    write_test_image(image_a_path);
    write_test_image(image_b_path);
    const auto matches = make_match_set(torch::empty({0, 2}, torch::kFloat32), torch::empty({0, 2}, torch::kFloat32),
...
static void visualization_draws_sparse_match_indices()
{
    ...
    const pfm::MatchSet matches{torch::tensor({{1, 0}}, torch::kInt64), torch::tensor({1.0F}, torch::kFloat32),
                                torch::empty({0, 2}, torch::kFloat32), torch::empty({0, 2}, torch::kFloat32),
                                torch::empty({0}, torch::kFloat32)};

    const auto output_path =
        pfm::save_match_visualization(image_a_path.string(), image_b_path.string(), features_a, features_b, matches,
                                      temp_dir.file("vis").string(), 10, 8, 10, 8);
    const auto output = cv::imread(output_path.string(), cv::IMREAD_COLOR);
...
static void visualization_scales_match_points_to_image_pixels()
{
    ...
    write_scaled_test_image(image_b_path);
    const auto matches =
        make_match_set(torch::tensor({{5.0F, 4.0F}}, torch::kFloat32), torch::tensor({{6.0F, 5.0F}}, torch::kFloat32),
...
static void visualization_colors_warp_correct_matches_green_and_wrong_matches_red()
{
    ...
    const auto output_path = pfm::save_match_visualization(image_a_path.string(), image_b_path.string(), matches,
                                                           temp_dir.file("vis").string(), warp, 2.0);
    const auto output = cv::imread(output_path.string(), cv::IMREAD_COLOR);
...
void register_visualization_tests()
{
    ...
    register_test("visualization writes feature png without keypoints",
                  visualization_writes_feature_png_without_keypoints);
    ...
    register_test("visualization scales match points to image pixels",
                  visualization_scales_match_points_to_image_pixels);
