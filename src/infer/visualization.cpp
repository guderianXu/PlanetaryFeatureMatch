...
torch::Tensor scaled_keypoints(const torch::Tensor& keypoints, int image_width, int image_height, int64_t map_width,
                               int64_t map_height)
{
    ...
    points.index_put_({torch::indexing::Slice(), 0}, points.index({torch::indexing::Slice(), 0}) *
                                                         static_cast<double>(image_width) /
                                                         static_cast<double>(map_width));
    points.index_put_({torch::indexing::Slice(), 1}, points.index({torch::indexing::Slice(), 1}) *
...
void validate_match_points(const torch::Tensor& points_a, const torch::Tensor& points_b,
...
{
    return (coordinate + 0.5) * static_cast<double>(warp_size) / static_cast<double>(std::max(1, image_size)) - 0.5;
}

bool is_warp_correct_match(const torch::Tensor& warp, double correct_threshold_pixels, float image_x_a, float image_y_a,
                           float image_x_b, float image_y_b, int image_a_width, int image_a_height, int image_b_width,
                           int image_b_height)
{
...
MatchSet scaled_match_set(const MatchSet& match_set, int image_a_width, int image_a_height, int image_b_width,
                          int image_b_height, int64_t map_a_width, int64_t map_a_height, int64_t map_b_width,
                          int64_t map_b_height)
{
    return MatchSet{match_set.sparse_matches, match_set.sparse_scores,
...
MatchSet make_sparse_match_points(const FeatureSet& features_a, const FeatureSet& features_b, const MatchSet& match_set,
                                  int image_a_width, int image_a_height, int image_b_width, int image_b_height,
                                  int64_t map_a_width, int64_t map_a_height, int64_t map_b_width, int64_t map_b_height)
{
    ...
    const auto keypoints_a =
        scaled_keypoints(features_a.keypoints, image_a_width, image_a_height, map_a_width, map_a_height);
    const auto keypoints_b =
        scaled_keypoints(features_b.keypoints, image_b_width, image_b_height, map_b_width, map_b_height);
    if (matches.size(0) == 0)
    {
        const auto float_options = torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCPU);
        return MatchSet{matches, scores, torch::empty({0, 2}, float_options), torch::empty({0, 2}, float_options),
                        scores};
    }
    return MatchSet{matches, scores,
                    keypoints_a.index_select(0, matches.index({torch::indexing::Slice(), 0})).contiguous(),
                    keypoints_b.index_select(0, matches.index({torch::indexing::Slice(), 1})).contiguous(), scores};
}

MatchSet concatenate_match_points(const MatchSet& sparse_points, const MatchSet& dense_points)
{
    return MatchSet{sparse_points.sparse_matches, sparse_points.sparse_scores,
...
void draw_matches(cv::Mat& canvas, int image_b_offset, int image_a_width, int image_a_height, int image_b_width,
                  int image_b_height, const MatchSet& match_set, const torch::Tensor* warp_a_to_b,
                  double correct_threshold_pixels)
{
    ...
    for (const auto index : sorted_match_indices(confidence))
    {
        ...
        const cv::Scalar color =
            use_warp_colors
                ? (is_warp_correct_match(warp, correct_threshold_pixels, image_x_a, image_y_a, image_x_b, image_y_b,
                                         image_a_width, image_a_height, image_b_width, image_b_height)
                       ? cv::Scalar(0, 220, 0)
                       : cv::Scalar(0, 0, 255))
                : cv::Scalar(0, 80.0 + 175.0 * score, 255.0 * score);
        const cv::Point point_a(static_cast<int>(std::round(image_x_a)), static_cast<int>(std::round(image_y_a)));
        const cv::Point point_b(image_b_offset + static_cast<int>(std::round(image_x_b)),
                                                 ...
                                                 int64_t feature_map_height)
{
    auto image = read_color_image(image_path);
    draw_keypoints(
        image, scaled_keypoints(feature_set.keypoints, image.cols, image.rows, feature_map_width, feature_map_height));
    const auto output_path = std::filesystem::path(visualization_dir) / (sanitized_stem(image_path) + "_features.png");
    write_png(output_path, image);
    return output_path;
}
                                               ...
                                               const torch::Tensor& warp_a_to_b, double correct_threshold_pixels)
{
    ...
    const auto image_b = read_color_image(image_b_path);
    auto canvas = make_side_by_side(image_a, image_b);
    draw_matches(canvas, image_a.cols, image_a.cols, image_a.rows, image_b.cols, image_b.rows, match_set, &warp_a_to_b,
                                               ...
                                               int64_t feature_map_b_width, int64_t feature_map_b_height)
{
    ...
    const auto image_b = read_color_image(image_b_path);
    auto canvas = make_side_by_side(image_a, image_b);
    draw_matches(canvas, image_a.cols, image_a.cols, image_a.rows, image_b.cols, image_b.rows,
                 scaled_match_set(match_set, image_a.cols, image_a.rows, image_b.cols, image_b.rows,
                                  feature_map_a_width, feature_map_a_height, feature_map_b_width, feature_map_b_height),
                 nullptr, 0.0);
    const auto output_path = std::filesystem::path(visualization_dir) /
                             (sanitized_stem(image_a_path) + "__" + sanitized_stem(image_b_path) + "_matches.png");
    write_png(output_path, canvas);
                                               ...
                                               const torch::Tensor& warp_a_to_b, double correct_threshold_pixels)
{
    ...
    draw_matches(canvas, image_a.cols, image_a.cols, image_a.rows, image_b.cols, image_b.rows,
                 scaled_match_set(match_set, image_a.cols, image_a.rows, image_b.cols, image_b.rows,
                                  feature_map_a_width, feature_map_a_height, feature_map_b_width, feature_map_b_height),
                 &warp_a_to_b, correct_threshold_pixels);
    const auto output_path = std::filesystem::path(visualization_dir) /
                             (sanitized_stem(image_a_path) + "__" + sanitized_stem(image_b_path) + "_matches.png");
    write_png(output_path, canvas);
    return output_path;
                                               ...
                                               int64_t feature_map_b_width, int64_t feature_map_b_height)
{
    ...
    const auto sparse_points = make_sparse_match_points(
        features_a, features_b, match_set, image_a.cols, image_a.rows, image_b.cols, image_b.rows, feature_map_a_width,
        feature_map_a_height, feature_map_b_width, feature_map_b_height);
    const auto dense_points =
        scaled_match_set(match_set, image_a.cols, image_a.rows, image_b.cols, image_b.rows, feature_map_a_width,
                         feature_map_a_height, feature_map_b_width, feature_map_b_height);
    auto canvas = make_side_by_side(image_a, image_b);
    draw_matches(canvas, image_a.cols, image_a.cols, image_a.rows, image_b.cols, image_b.rows,
                 concatenate_match_points(sparse_points, dense_points), nullptr, 0.0);
    const auto output_path = std::filesystem::path(visualization_dir) /
                             (sanitized_stem(image_a_path) + "__" + sanitized_stem(image_b_path) + "_matches.png");
    write_png(output_path, canvas);
    return output_path;
                                               ...
                                               const torch::Tensor& warp_a_to_b, double correct_threshold_pixels)
{
    ...
    auto canvas = make_side_by_side(image_a, image_b);
    draw_matches(canvas, image_a.cols, image_a.cols, image_a.rows, image_b.cols, image_b.rows,
                 concatenate_match_points(sparse_points, dense_points), &warp_a_to_b, correct_threshold_pixels);
    const auto output_path = std::filesystem::path(visualization_dir) /
                             (sanitized_stem(image_a_path) + "__" + sanitized_stem(image_b_path) + "_matches.png");
    write_png(output_path, canvas);
    return output_path;
