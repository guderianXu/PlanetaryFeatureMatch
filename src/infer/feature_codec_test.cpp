...
{
    return pfm::FeatureSet{torch::tensor({{1.0F, 2.0F}, {3.0F, 4.0F}}, torch::kFloat32),
...
static void feature_codec_round_trips_all_fields()
{
    ...
    auto expected = makeFeatureSet();
    expected.feature_map_width = 12;
    expected.feature_map_height = 10;
    ...
    PFM_REQUIRE(actual.feature_map_width == expected.feature_map_width);
    PFM_REQUIRE(actual.feature_map_height == expected.feature_map_height);
}
