...
{
    return pfm::RawFeatureMaps{heatmap,
...
static void decode_sparse_features_gathers_values_at_top_k_points()
{
    ...
    maps.orientation = torch::tensor(
        {{{{10.0F, 20.0F, 30.0F}, {40.0F, 50.0F, 60.0F}}, {{11.0F, 21.0F, 31.0F}, {41.0F, 51.0F, 61.0F}}}},
        torch::kFloat32);
    maps.affine = torch::tensor({{{{1.0F, 2.0F, 3.0F}, {4.0F, 5.0F, 6.0F}},
...
static void decode_feature_maps_relaxes_nms_to_reach_min_keypoints_when_possible()
{
    auto heatmap = torch::zeros({1, 1, 1, 4}, torch::kFloat32);
    heatmap.index_put_({0, 0, 0, 0}, 4.0F);
    heatmap.index_put_({0, 0, 0, 1}, 3.0F);
    heatmap.index_put_({0, 0, 0, 2}, 2.0F);
    heatmap.index_put_({0, 0, 0, 3}, 1.0F);
    const auto maps = makeUniformMaps(heatmap);
    pfm::FeatureDecodeConfig config;
    config.max_keypoints = 4;
    config.min_keypoints = 3;
    config.semi_dense_threshold = 0.5;
    config.keypoint_grid_rows = 1;
    config.keypoint_grid_cols = 1;
    config.keypoints_per_cell = 4;
    config.nms_radius = 2;
    ...
    PFM_REQUIRE(features.keypoints.sizes() == torch::IntArrayRef({3, 2}));
    PFM_REQUIRE_CLOSE(features.keypoints.index({0, 0}).item<float>(), 0.0F, 1.0e-6F);
    PFM_REQUIRE(features.keypoints.index({torch::indexing::Slice(), 0}).eq(1.0F).any().item<bool>());
}

static void decode_feature_maps_caps_min_keypoints_at_available_valid_candidates()
{
    ...
    config.keypoints_per_cell = 4;
    config.nms_radius = 0;
    auto mask = torch::tensor({1.0F, 1.0F, 0.0F, 0.0F}, torch::kFloat32).reshape({1, 4});

    const auto features = pfm::decode_feature_maps(maps, config, mask);
...
static void decode_feature_maps_pools_descriptors_along_predicted_orientation()
{
    auto heatmap = torch::zeros({1, 1, 3, 3}, torch::kFloat32);
    heatmap.index_put_({0, 0, 1, 1}, 10.0F);
    auto maps = makeUniformMaps(heatmap);
    maps.descriptors = torch::zeros({1, 2, 3, 3}, torch::kFloat32);
    maps.descriptors.index_put_({0, 0, 1, 1}, 2.0F);
    maps.descriptors.index_put_({0, 1, 1, 2}, 2.0F);
    maps.orientation = torch::zeros({1, 2, 3, 3}, torch::kFloat32);
    maps.orientation.index_put_({0, 0, torch::indexing::Slice(), torch::indexing::Slice()}, 1.0F);
    pfm::FeatureDecodeConfig config;
    ...
    config.keypoints_per_cell = 1;
    config.nms_radius = 0;
    config.descriptor_pool_radius = 1;

    const auto features = pfm::decode_feature_maps(maps, config, torch::Tensor());

    PFM_REQUIRE(features.descriptors.sizes() == torch::IntArrayRef({1, 2}));
    PFM_REQUIRE_CLOSE(features.descriptors.index({0, 0}).item<float>(), 0.992278F, 1.0e-5F);
...
static void decode_feature_maps_canonicalizes_four_way_descriptor_orientation()
{
    ...
    heatmap.index_put_({0, 0, 0, 0}, 10.0F);
    auto maps = makeUniformMaps(heatmap);
    maps.descriptors = torch::tensor({{{{0.0F}}, {{1.0F}}, {{0.0F}}, {{0.0F}}}}, torch::kFloat32);
    maps.orientation = torch::tensor({{{{0.0F}}, {{1.0F}}}}, torch::kFloat32);
    pfm::FeatureDecodeConfig config;
    config.max_keypoints = 1;
    config.semi_dense_threshold = 0.5;
    config.keypoint_grid_rows = 1;
    ...
    config.keypoints_per_cell = 1;
    config.nms_radius = 0;
    config.descriptor_orientation_canonicalization = true;
...
{
    register_test("decode_sparse_features_gathers_values_at_top_k_points",
                  decode_sparse_features_gathers_values_at_top_k_points);
    register_test("decode_feature_maps_excludes_masked_sparse_locations",
                  decode_feature_maps_excludes_masked_sparse_locations);
    register_test("decode_feature_maps_returns_zero_score_sparse_keypoints",
                  decode_feature_maps_returns_zero_score_sparse_keypoints);
    register_test("decode_feature_maps_returns_empty_sparse_features_when_mask_is_empty",
    ...
    register_test("decode_feature_maps_avoids_row_major_grid_truncation_bias",
                  decode_feature_maps_avoids_row_major_grid_truncation_bias);
    register_test("decode_feature_maps_relaxes_nms_to_reach_min_keypoints_when_possible",
                  decode_feature_maps_relaxes_nms_to_reach_min_keypoints_when_possible);
    register_test("decode_feature_maps_caps_min_keypoints_at_available_valid_candidates",
                  decode_feature_maps_caps_min_keypoints_at_available_valid_candidates);
    register_test("decode_feature_maps_allows_zero_score_candidate_in_each_grid_cell",
                  decode_feature_maps_allows_zero_score_candidate_in_each_grid_cell);
    register_test("decode_feature_maps_keeps_zero_score_grid_cells_when_positive_scores_exist",
    ...
    register_test("decode_dense_features_returns_empty_tensors_when_no_confidence_matches",
                  decode_dense_features_returns_empty_tensors_when_no_confidence_matches);
    register_test("decode_feature_maps_returns_cpu_float_contiguous_tensors",
