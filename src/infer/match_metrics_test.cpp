...
pfm::FeatureSet makeMetricFeatureSet(const torch::Tensor& sparse_points, const torch::Tensor& dense_points) {
    ...
    pfm::FeatureSet features{
        sparse_points.to(torch::kFloat32),
...
static void match_metrics_scores_sparse_and_dense_matches_against_warp() {
    const auto features_a = makeMetricFeatureSet(
        torch::tensor({{1.0F, 1.0F}, {2.0F, 2.0F}}),
        torch::tensor({{0.0F, 0.0F}, {3.0F, 3.0F}}));
    const auto features_b = makeMetricFeatureSet(
        torch::tensor({{1.0F, 1.0F}, {3.0F, 3.0F}}),
        torch::tensor({{0.0F, 0.0F}, {1.0F, 1.0F}}));
    const pfm::MatchSet matches{
        torch::tensor({{0, 0}, {1, 1}}, torch::kInt64),
        torch::tensor({1.0F, 0.5F}),
        torch::tensor({{0.0F, 0.0F}, {3.0F, 3.0F}}),
        torch::tensor({{0.0F, 0.0F}, {1.0F, 1.0F}}),
        torch::tensor({1.0F, 0.5F})};
...
static void match_metrics_reports_keypoint_coverage_and_descriptor_rank() {
    auto features_a = makeMetricFeatureSet(
        torch::tensor({{0.0F, 0.0F}, {2.0F, 2.0F}}),
        torch::empty({0, 2}, torch::kFloat32));
    auto features_b = makeMetricFeatureSet(
        torch::tensor({{0.0F, 0.0F}, {2.0F, 2.0F}, {3.0F, 3.0F}}),
        torch::empty({0, 2}, torch::kFloat32));
    features_a.descriptors = torch::tensor(
        {{1.0F, 0.0F, 0.0F, 0.0F}, {0.0F, 1.0F, 0.0F, 0.0F}},
        torch::kFloat32);
    features_b.descriptors = torch::tensor(
