...
static void graph_matching_loss_accepts_dustbin_targets() {
    auto logits = torch::tensor(
        {{5.0F, -1.0F, -2.0F}, {-1.0F, 5.0F, -2.0F}, {-2.0F, -2.0F, 4.0F}},
        torch::kFloat32);
...
static void confidence_bce_loss_replaces_non_finite_probabilities() {
    auto confidence = torch::tensor({
        std::numeric_limits<float>::quiet_NaN(),
        std::numeric_limits<float>::infinity(),
        -std::numeric_limits<float>::infinity(),
        0.75F}, torch::kFloat32);
...
void register_loss_tests() {
    ...
    register_test(
        "masked smooth l1 loss uses quadratic penalty near zero",
    ...
    register_test(
        "confidence bce loss clamps probabilities to valid range",
        confidence_bce_loss_clamps_probabilities_to_valid_range);
    register_test(
        "confidence bce loss replaces non finite probabilities",
