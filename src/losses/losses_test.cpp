...
static void descriptor_candidate_loss_rejects_channel_shifted_negative()
{
    auto queries = torch::tensor({{{1.0F, 0.0F, 0.0F, 0.0F}}}, torch::kFloat32);
...
static void graph_matching_loss_accepts_dustbin_targets()
{
    auto logits = torch::tensor({{5.0F, -1.0F, -2.0F}, {-1.0F, 5.0F, -2.0F}, {-2.0F, -2.0F, 4.0F}}, torch::kFloat32);
    auto target = torch::tensor({0, 1}, torch::kLong);

    auto loss = pfm::graph_matching_cross_entropy_loss(logits, target);

    PFM_REQUIRE(loss.item<float>() < 0.05F);
}

static void graph_matching_loss_rejects_target_without_dustbin_range()
{
    auto logits = torch::zeros({3, 3}, torch::kFloat32);
    auto target = torch::tensor({0, 3}, torch::kLong);

    PFM_REQUIRE_THROWS_AS(pfm::graph_matching_cross_entropy_loss(logits, target), std::invalid_argument);
}
...
static void masked_smooth_l1_loss_uses_quadratic_penalty_near_zero()
{
    auto pred = torch::tensor({{{{0.5F, 2.0F}}}}, torch::kFloat32);
    auto target = torch::zeros_like(pred);
    auto mask = torch::ones({1, 1, 1, 2}, torch::kFloat32);

    auto loss = pfm::masked_smooth_l1_loss(pred, target, mask);

    PFM_REQUIRE_CLOSE(loss.item<float>(), 0.8125F, 1.0e-6F);
}
...
static void confidence_bce_loss_clamps_probabilities_to_valid_range()
{
    auto confidence = torch::tensor({-0.25F, 1.25F}, torch::kFloat32);
    auto target = torch::tensor({0.0F, 1.0F}, torch::kFloat32);
...
static void confidence_bce_loss_replaces_non_finite_probabilities()
{
    auto confidence = torch::tensor({std::numeric_limits<float>::quiet_NaN(), std::numeric_limits<float>::infinity(),
                                     -std::numeric_limits<float>::infinity(), 0.75F},
                                    torch::kFloat32);
    auto target = torch::tensor({0.0F, 1.0F, 0.0F, 1.0F}, torch::kFloat32);
...
void register_loss_tests()
{
    ...
    register_test("descriptor candidate loss rejects channel shifted negative",
                  descriptor_candidate_loss_rejects_channel_shifted_negative);
    ...
    register_test("graph matching loss accepts dustbin targets", graph_matching_loss_accepts_dustbin_targets);
    register_test("graph matching loss rejects target without dustbin range",
                  graph_matching_loss_rejects_target_without_dustbin_range);
    register_test("descriptor loss rejects non long labels", descriptor_loss_rejects_non_long_labels);
    register_test("descriptor loss rejects descriptor dtype mismatch",
                  descriptor_loss_rejects_descriptor_dtype_mismatch);
    register_test("descriptor loss rejects descriptor batch mismatch",
                  descriptor_loss_rejects_descriptor_batch_mismatch);
    register_test("descriptor loss rejects descriptor dimension mismatch",
    ...
    register_test("descriptor loss rejects target shape with wrong batch",
                  descriptor_loss_rejects_target_shape_with_wrong_batch);
    register_test("descriptor loss rejects target shape with wrong count",
    ...
    register_test("masked l1 loss averages channels for selected spatial position",
    ...
    register_test("masked smooth l1 loss uses quadratic penalty near zero",
                  masked_smooth_l1_loss_uses_quadratic_penalty_near_zero);
    register_test("masked l1 loss rejects ambiguous bhw mask", masked_l1_loss_rejects_ambiguous_bhw_mask);
    register_test("confidence bce loss returns finite positive scalar for matching shapes",
    ...
    register_test("confidence bce loss clamps probabilities to valid range",
                  confidence_bce_loss_clamps_probabilities_to_valid_range);
    register_test("confidence bce loss replaces non finite probabilities",
                  confidence_bce_loss_replaces_non_finite_probabilities);
    register_test("confidence bce loss rejects incompatible target shape",
