...
void validate_graph_features(const torch::Tensor& features_a, const torch::Tensor& features_b, int64_t hidden_dim)
{
...
void validate_matcher_inputs(const torch::Tensor& descriptors_a, const torch::Tensor& keypoints_a,
...
torch::Tensor prepare_keypoints_for_embedding(const torch::Tensor& keypoints)
{
    ...
    return torch::cat({radius, radius.pow(2)}, 1);
}
...
std::pair<torch::Tensor, torch::Tensor> PlanetaryGraphAttentionLayerImpl::forward(const torch::Tensor& features_a,
                                                                                  const torch::Tensor& features_b)
{
    ...
    return {_feed_forward_norm(refined_a + _feed_forward->forward(refined_a)),
            _feed_forward_norm(refined_b + _feed_forward->forward(refined_b))};
}

PlanetaryGraphMatcherImpl::PlanetaryGraphMatcherImpl(int64_t descriptor_dim, int64_t hidden_dim,
...
PlanetaryGraphMatcherOutput PlanetaryGraphMatcherImpl::forward(const torch::Tensor& descriptors_a,
                                                               ...
                                                               const torch::Tensor& keypoints_b)
{
    ...
    embed_a = torch::nn::functional::normalize(_score_projection(embed_a),
                                               torch::nn::functional::NormalizeFuncOptions().p(2).dim(1));
    embed_b = torch::nn::functional::normalize(_score_projection(embed_b),
                                               torch::nn::functional::NormalizeFuncOptions().p(2).dim(1));
    ...
    logits.index_put_(
        {torch::indexing::Slice(0, descriptors_a.size(0)), torch::indexing::Slice(0, descriptors_b.size(0))},
        pair_logits);
    ...
    return PlanetaryGraphMatcherOutput{logits.contiguous(), matches,
                                       probabilities.to(torch::kCPU, torch::kFloat32).contiguous()};
}
