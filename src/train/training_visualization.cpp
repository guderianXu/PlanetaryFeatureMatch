...
cv::Mat tensor_to_bgr_u8(const torch::Tensor& image) {
    ...
    if (tensor.dim() == 2) {
        cv::Mat gray(static_cast<int>(tensor.size(0)), static_cast<int>(tensor.size(1)), CV_8UC1, tensor.data_ptr<uint8_t>());
        cv::Mat color;
    ...
    cv::Mat rgb(static_cast<int>(tensor.size(0)), static_cast<int>(tensor.size(1)), CV_8UC3, tensor.data_ptr<uint8_t>());
    cv::Mat bgr;
    cv::cvtColor(rgb, bgr, cv::COLOR_RGB2BGR);
    return bgr.clone();
}

void write_image_job(
...
void AsyncVisualizationWriter::enqueueImage(
    const std::filesystem::path& output_path,
    const torch::Tensor& image,
    const std::string& overlay
) {
    ...
    enqueueJob([output_path, cpu_image, overlay]() { write_image_job(output_path, cpu_image, overlay); });
}
...
void AsyncVisualizationWriter::enqueueJob(std::function<void()> job) {
    ...
    _not_full.wait(lock, [this]() { return _jobs.size() < _capacity || _error != nullptr; });
    if (_error) {
...
void AsyncVisualizationWriter::run() {
    try {
        while (true) {
            ...
            {
                std::unique_lock<std::mutex> lock(_mutex);
                _not_empty.wait(lock, [this]() { return _stopping || !_jobs.empty(); });
                if (_jobs.empty()) {
                    return;
                }
