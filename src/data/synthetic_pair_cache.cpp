...
std::filesystem::path source_dir_path(
...
std::filesystem::path pair_pt_path(
    const std::string& cache_dir,
    std::size_t index,
...
std::filesystem::path pair_view_path(
    const std::string& cache_dir,
    const ImageDataset& dataset,
    std::size_t index,
...
std::filesystem::path named_rotation_image_path(
    const std::string& cache_dir,
    const ImageDataset& dataset,
    std::size_t source_index,
    float degrees
...
std::filesystem::path named_rotation_pair_path(
    const std::string& cache_dir,
    const ImageDataset& dataset,
    std::size_t source_index,
    float degrees
...
std::filesystem::path source_view_a_path(
    const std::string& cache_dir,
    const ImageDataset& dataset,
    std::size_t source_index
) {
...
bool manifest_matches(const SyntheticPairCacheConfig& config) {
    try {
        ...
        return read_int64(archive, "format_version") == CACHE_FORMAT_VERSION &&
               ...
               float_matches(read_float(archive, "rotation_step_degrees"),
                             config.pair_config.rotation_step_degrees) &&
...
void generate_cache(const ImageDataset& dataset, const SyntheticPairCacheConfig& config) {
    ...
    for (std::size_t index = 0; index < config.pair_count; ++index) {
        ...
        if (pair_config.augmentation_profile == SyntheticPairAugmentationProfile::RotationOnly) {
            ...
            write_pair_archive(pair, source_index, named_rotation_pair_path(config.cache_dir, dataset, source_index, degrees));
        }
    }
...
SyntheticPairCacheTensorDataset::SyntheticPairCacheTensorDataset(std::string cache_dir)
    : _cache(std::move(cache_dir)) {}
