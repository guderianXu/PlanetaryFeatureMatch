...
static void synthetic_pair_cache_generates_pt_manifest_and_png_views() {
    ...
    PFM_REQUIRE(std::filesystem::exists(std::filesystem::path(config.cache_dir) / "pair_000000.pt"));
    PFM_REQUIRE(std::filesystem::exists(std::filesystem::path(config.cache_dir) / "source_000000_view_a.png"));
    PFM_REQUIRE(std::filesystem::exists(std::filesystem::path(config.cache_dir) / "pair_000000_view_b.png"));
    PFM_REQUIRE(std::filesystem::exists(std::filesystem::path(config.cache_dir) / "pair_000001.pt"));
}
...
static void synthetic_pair_cache_generates_multiple_pairs_per_source_image() {
    ...
    PFM_REQUIRE(std::filesystem::exists(std::filesystem::path(config.cache_dir) / "pair_000002.pt"));
    PFM_REQUIRE(std::filesystem::exists(std::filesystem::path(config.cache_dir) / "pair_000003.pt"));
    PFM_REQUIRE(std::filesystem::exists(std::filesystem::path(config.cache_dir) / "source_000001_view_a.png"));
    PFM_REQUIRE(std::filesystem::exists(std::filesystem::path(config.cache_dir) / "pair_000003_view_b.png"));
}
