...
static float mean_warp_displacement(const pfm::SyntheticPair& pair) {
    const auto grid = torch::stack(
        torch::meshgrid({torch::arange(pair.warp_a_to_b.size(0), torch::kFloat32),
                         torch::arange(pair.warp_a_to_b.size(1), torch::kFloat32)}, "ij"),
        2);
...
void register_synthetic_pair_tests() {
    ...
    register_test("synthetic pair warp matches shifted view content",
                  synthetic_pair_warp_matches_shifted_view_content);
    register_test("synthetic pair warp matches scaled view content",
                  synthetic_pair_warp_matches_scaled_view_content);
