...
{
    return fields.size() >= 5 && fields[0] == "pair_id" && fields[1] == "feature_a" && fields[2] == "feature_b" &&
           fields[3] == "matches" && fields[4] == "warp_a_to_b";
}
...
std::vector<PairManifestEntry> parsePairManifest(std::istream& input)
{
    ...
    while (std::getline(input, line))
    {
        ...
        entries.push_back(PairManifestEntry{fields[0], fields[1], fields[2], fields[3], fields[4]});
    }
    if (entries.empty())
    {
...
std::string pairManifestCsv(const std::vector<PairManifestEntry>& entries)
{
    ...
    {
        if (entry.pair_id.empty() || entry.feature_a.empty() || entry.feature_b.empty() || entry.matches.empty() ||
            entry.warp_a_to_b.empty())
        {
            throw std::invalid_argument("cache eval manifest entry contains an empty required field");
        }
        out << csvEscape(entry.pair_id) << ',' << csvEscape(entry.feature_a) << ',' << csvEscape(entry.feature_b) << ','
            << csvEscape(entry.matches) << ',' << csvEscape(entry.warp_a_to_b) << '\n';
    }
    return out.str();
}
