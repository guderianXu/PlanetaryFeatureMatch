...
void PairMetrics::addDescriptorQueries(int64_t queries, int64_t top1_correct, int64_t rank_observed, int64_t rank_sum)
{
...
std::string PairMetrics::csvRow() const
{
    ...
    out << std::setprecision(12) << csvEscape(_pair_id) << ',' << _total_matches << ',' << _correct_matches << ','
        << precision() << ',' << _features_a << ',' << _features_b << ',' << _matched_features_a << ','
        << _matched_features_b << ',' << coverageA() << ',' << coverageB() << ',' << _source_features << ','
        << _valid_warp_features << ',' << _covered_features << ',' << featureCoverage() << ',' << _descriptor_queries
        << ',' << _descriptor_top1 << ',' << descriptorTop1Accuracy() << ',' << _descriptor_rank_observed << ','
        << _descriptor_rank_sum << ',' << meanDescriptorRank();
    return out.str();
}
...
PairMetrics MetricsAccumulator::summary() const
{
    ...
    for (const auto& pair : _pairs)
    {
        ...
        aggregate.setFeatureCounts(aggregate.featureCountA() + pair.featureCountA(),
                                   aggregate.featureCountB() + pair.featureCountB());
        aggregate.setMatchedFeatureCounts(aggregate.matchedFeatureCountA() + pair.matchedFeatureCountA(),
                                          aggregate.matchedFeatureCountB() + pair.matchedFeatureCountB());
        aggregate.setFeatureCoverage(aggregate.sourceFeatureCount() + pair.sourceFeatureCount(),
                                     aggregate.validWarpFeatureCount() + pair.validWarpFeatureCount(),
                                     aggregate.coveredFeatureCount() + pair.coveredFeatureCount());
        aggregate.addDescriptorQueries(pair.descriptorQueries(), pair.descriptorTop1Count(),
                                       pair.descriptorRankObserved(), pair.descriptorRankSum());
    }
    return aggregate;
}
