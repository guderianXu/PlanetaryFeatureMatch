...
QualityDecision evaluatePairQuality(const PairMetrics& pair, const QualityThresholds& thresholds) {
    ...
    if (const auto deficit =
            ratioDeficit(pair.descriptorTop1Accuracy(), thresholds.min_descriptor_top1_accuracy);
        deficit > 0.0) {
...
std::vector<HardPair> selectHardPairs(
    const std::vector<PairMetrics>& pairs,
    ...
    std::sort(hard_pairs.begin(), hard_pairs.end(), [](const HardPair& lhs, const HardPair& rhs) {
...
std::string qualityDecisionsCsv(
    const std::vector<PairMetrics>& pairs,
    const QualityThresholds& thresholds
) {
    std::ostringstream out;
