...
pfm::MatchSet makeEvalMatchSet(
    const torch::Tensor& sparse_scores,
    ...
    return pfm::MatchSet{
        torch::zeros({sparse_scores.size(0), 2}, long_options),
...
static void eval_pipeline_aggregates_known_metrics() {
    const std::vector<std::pair<pfm::FeatureSet, pfm::FeatureSet>> feature_sets = {
        ...
        {makeEvalFeatureSet(5), makeEvalFeatureSet(2)}};
    const std::vector<pfm::MatchSet> match_sets = {
        makeEvalMatchSet(torch::tensor({0.5F, 1.0F}, torch::kFloat32), torch::tensor({0.25F, 0.75F}, torch::kFloat32), 2),
