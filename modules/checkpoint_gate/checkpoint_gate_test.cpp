...
static void checkpoint_gate_parses_match_cli_metrics()
{
    const auto metrics = pfm::parse_checkpoint_gate_metrics(
        "matching complete: sparse_matches=421 correct_matches=421 wrong_matches=0 "
        "match_precision=1 graph_work=0.35 elapsed=0.895s");
    ...
    PFM_REQUIRE_CLOSE(metrics.graph_attention_work_fraction, 0.35, 1.0e-9);
}
...
static void checkpoint_gate_rejects_excessive_graph_work()
{
    const pfm::CheckpointGateMetrics metrics{421, 0, 1.0, 0.82};
    pfm::CheckpointGateThreshold threshold;
    threshold.min_correct_matches = 400;
    threshold.min_precision = 0.99;
    threshold.max_graph_attention_work_fraction = 0.5;
    const auto decision = pfm::evaluate_checkpoint_gate_metrics(metrics, threshold);
    PFM_REQUIRE(!decision.passed);
    PFM_REQUIRE(decision.reason.find("graph_work") != std::string::npos);
}
...
static void checkpoint_gate_accepts_old_match_cli_output_without_graph_work()
{
    const auto metrics = pfm::parse_checkpoint_gate_metrics(
        "matching complete: sparse_matches=421 correct_matches=421 wrong_matches=0 match_precision=1 elapsed=0.895s");
    PFM_REQUIRE(metrics.correct_matches == 421);
    PFM_REQUIRE_CLOSE(metrics.graph_attention_work_fraction, 0.0, 1.0e-9);
}
...
void register_checkpoint_gate_tests()
{
    ...
    register_test("checkpoint_gate_rejects_excessive_graph_work", checkpoint_gate_rejects_excessive_graph_work);
    register_test("checkpoint_gate_accepts_metrics_that_meet_thresholds",
                  checkpoint_gate_accepts_metrics_that_meet_thresholds);
    register_test("checkpoint_gate_accepts_old_match_cli_output_without_graph_work",
                  checkpoint_gate_accepts_old_match_cli_output_without_graph_work);
}
