...
double normalizedBoundingBoxArea(
...
double displacementConcentration(
    const std::vector<MatchPoint>& matches,
    ImageExtent image_a,
    ImageExtent image_b,
    double bin_fraction
    ...
    for (const auto& match : matches) {
        ...
        const BinKey key{
            static_cast<long long>(std::floor(dx / bin_size)),
...
SpatialMatchQuality evaluateSpatialMatchQuality(
    ...
    quality.displacement_concentration = displacementConcentration(
        matches,
        image_a,
        image_b,
        options.displacement_bin_fraction
    );
    ...
    const bool narrow_bbox =
        quality.normalized_spatial_coverage_a <= options.narrow_bbox_area_threshold &&
        quality.normalized_spatial_coverage_b <= options.narrow_bbox_area_threshold;
    const bool narrow_grid =
        quality.grid_coverage_a <= options.narrow_grid_coverage_threshold &&
