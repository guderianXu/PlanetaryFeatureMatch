...
AsyncDataLoader::AsyncDataLoader(std::shared_ptr<TensorDataset> dataset, std::unique_ptr<Sampler> sampler,
                                 TensorBatchCollator collator, DataLoaderOptions options)
    : _dataset(std::move(dataset)), _sampler(std::move(sampler)), _collator(std::move(collator)), _options(options)
{
    if (!_dataset)
    {
