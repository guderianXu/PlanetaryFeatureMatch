...
DescriptorTopKDecision makeDecision(
    const DescriptorTopKBand band,
    const int top_k,
    const bool prefer_reciprocal_check,
    const bool request_geometric_retry,
    std::string reason
) {
...
DescriptorTopKDecision selectDescriptorTopKPolicy(
    ...
    const bool noisy_candidate_pool =
        observation.tentative_match_count >= config.healthy_match_count &&
        verified_fraction < config.weak_verified_fraction;
    if (noisy_candidate_pool) {
        return makeDecision(
            DescriptorTopKBand::Conservative,
            config.conservative_top_k,
            true,
            true,
    ...
    const bool insufficient_match_support =
        observation.verified_match_count < config.minimum_viable_matches ||
        observation.spatial_coverage < config.weak_spatial_coverage;
    if (insufficient_match_support) {
        return makeDecision(
            DescriptorTopKBand::Loose,
            config.loose_top_k,
            true,
            true,
            "insufficient_match_support"
        );
    }

    const bool stable_high_confidence =
        observation.verified_match_count >= config.healthy_match_count &&
    ...
    return makeDecision(
        DescriptorTopKBand::Balanced,
        config.balanced_top_k,
        true,
        false,
        "balanced_default"
    );
