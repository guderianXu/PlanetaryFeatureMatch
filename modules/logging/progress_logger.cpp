...
{
    const int filled =
        metric.total_iterations > 0 ? std::min(_bar_width, metric.iteration * _bar_width / metric.total_iterations) : 0;
    ...
    _stream << "] " << metric.iteration << '/' << metric.total_iterations
            << " loss=" << metricValue(metric, "loss_total") << " match=" << metricValue(metric, "matcher_loss")
            << " feat=" << metricValue(metric, "feature_loss") << " rep=" << metricValue(metric, "repeatability_loss")
            << " dense=" << metricValue(metric, "dense_loss") << " off=" << metricValue(metric, "offset_error_px")
            << " desc_acc=" << metricValue(metric, "descriptor_accuracy")
            << " div=" << metricValue(metric, "descriptor_diversity") << " lr=" << metric.learning_rate;
