...
int main(int argc, char** argv)
{
    try
    {
        ...
        if (warp.size(0) != image_a.size(0) || warp.size(1) != image_a.size(1) || image_b.size(0) != image_a.size(0) ||
            image_b.size(1) != image_a.size(1))
        {
        ...
        for (int64_t y = 0; y < height; ++y)
        {
            for (int64_t x = 0; x < width; ++x)
            {
                ...
                const bool in_bounds = target_x >= 0.0F && target_x <= static_cast<float>(width - 1) &&
        ...
        std::cout << "total_pixels=" << total_pixels << " source_bright=" << source_bright
                  << " warp_in_bounds=" << warp_in_bounds << " bright_source_in_bounds=" << bright_source_in_bounds
