...
Options parseOptions(int argc, char** argv)
{
    ...
    for (int index = 1; index < argc; ++index)
    {
        ...
        {
            std::cout << "Usage: pfm_rotation_sweep_eval --image img/100.tif --checkpoint model.pt [options]\n"
...
double descriptorScore(const torch::Tensor& descriptors_a, int64_t index_a, const torch::Tensor& descriptors_b,
                       int64_t index_b)
{
    double dot = 0.0;
    double norm_a = 0.0;
...
AngleResult evaluateAngle(int angle, const pfm::FeatureSet& source_features, const pfm::FeatureSet& rotated_features,
                          const pfm::MatchSet& matches, double threshold_px)
{
    AngleResult result;
    result.angle = angle;
    ...
    for (int64_t row = 0; row < result.matches; ++row)
    {
        ...
        const auto expected = rotatePoint(ax, ay, static_cast<double>(rotated_features.feature_map_width),
                                          static_cast<double>(rotated_features.feature_map_height), angle);
    ...
    if (!errors.empty())
    {
        const auto passing = std::count_if(errors.begin(), errors.end(),
                                           [&](double error)
                                           {
...
DescriptorResult evaluateDescriptorMutualAngle(int angle, const pfm::FeatureSet& source_features,
                                               const pfm::FeatureSet& rotated_features, double threshold_px)
{
    ...
    const auto desc_a = torch::nn::functional::normalize(source_features.descriptors.to(torch::kCPU, torch::kFloat32),
                                                         torch::nn::functional::NormalizeFuncOptions().p(2).dim(1));
    const auto desc_b = torch::nn::functional::normalize(rotated_features.descriptors.to(torch::kCPU, torch::kFloat32),
                                                         torch::nn::functional::NormalizeFuncOptions().p(2).dim(1));
...
RepeatabilityResult evaluateRepeatability(int angle, const pfm::FeatureSet& source_features,
                                          const pfm::FeatureSet& rotated_features, double threshold_px)
{
...
std::string commonDecodeArgs(const Options& options)
{
    ...
    args << " --device " << shellQuote(options.device) << " --max-keypoints " << options.max_keypoints
         << " --min-keypoints " << options.min_keypoints << " --min-keypoint-intensity "
...
int main(int argc, char** argv)
{
    try
    {
        ...
        runCommand(shellQuote(options.pfm_cli) + " extract --image " + shellQuote(source_image.string()) +
                   " --checkpoint " + shellQuote(checkpoint.string()) + " --output " +
                   shellQuote(source_features.string()) + commonDecodeArgs(options));
        ...
        for (int angle = 0; angle < 360; angle += options.angle_step)
        {
            ...
            runCommand(shellQuote(options.pfm_cli) + " extract --image " + shellQuote(rotated_image.string()) +
                       " --checkpoint " + shellQuote(checkpoint.string()) + " --output " +
                       shellQuote(rotated_features.string()) + commonDecodeArgs(options));
            runCommand(shellQuote(options.pfm_cli) + " match --feature-a " + shellQuote(source_features.string()) +
