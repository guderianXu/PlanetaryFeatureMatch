...
int main(int argc, char** argv) {
    try {
        ...
        for (int64_t row = 0; row < sparse.size(0); ++row) {
            ...
            const auto [ax, ay] = map_feature_point_to_image_float(
                keypoints_a[ia],
                features_a.feature_map_width,
                features_a.feature_map_height,
                warp.size(1),
                warp.size(0));
            const auto [bx, by] = map_feature_point_to_image_float(
                keypoints_b[ib],
        ...
        for (std::size_t index = 0; index < rows.size(); ++index) {
            ...
            std::cerr << "score_prefix rank=" << prefix_total
                      << " row=" << rows[index].row
                      << " score=" << rows[index].score
                      << " correct=" << (rows[index].correct ? 1 : 0)
                      << " prefix_correct=" << prefix_correct
                      << " prefix_wrong=" << (prefix_total - prefix_correct)
                      << " prefix_precision=" << static_cast<double>(prefix_correct) /
