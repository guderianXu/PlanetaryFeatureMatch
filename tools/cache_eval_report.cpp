...
std::vector<pfm::cache_eval::PairManifestEntry> make_entries(const Options& options) {
    ...
    return {pfm::cache_eval::PairManifestEntry{
        options.pair_id,
        options.feature_a,
        options.feature_b,
        options.matches,
...
std::pair<int64_t, int64_t> count_unique_sparse_features(
    ...
    return {
        static_cast<int64_t>(used_a.size()),
        static_cast<int64_t>(used_b.size())};
}
...
std::vector<pfm::cache_eval::PairManifestEntry> select_hard_manifest_entries(
    ...
    for (const auto& hard_pair : hard_pairs) {
        const auto iter = std::find_if(entries.begin(), entries.end(), [&](const auto& entry) {
...
pfm::cache_eval::PairMetrics evaluate_pair(
    const pfm::cache_eval::PairManifestEntry& entry,
    double threshold
) {
    ...
    const auto match_metrics = pfm::compute_warp_match_metrics(
        features_a,
        features_b,
        matches,
        warp,
        threshold);
    const auto coverage_metrics = pfm::compute_warp_feature_coverage_metrics(
        features_a,
        features_b,
        warp,
        threshold);
    ...
    pair.setFeatureCoverage(
        coverage_metrics.source_total,
        coverage_metrics.valid_warp_total,
        coverage_metrics.covered_by_target_keypoint);
    pair.addDescriptorQueries(
        coverage_metrics.descriptor_rank_observed,
        coverage_metrics.descriptor_top1_count,
        coverage_metrics.descriptor_rank_observed,
        coverage_metrics.descriptor_rank_sum);
    return pair;
}
...
int main(int argc, char** argv) {
    try {
        ...
        if (!options.quality_output.empty()) {
            write_report(
                pfm::cache_eval::qualityDecisionsCsv(accumulator.pairs(), options.thresholds),
        ...
        if (!options.hard_output.empty() || !options.hard_manifest_output.empty() ||
            ...
            const auto hard_pairs = pfm::cache_eval::selectHardPairs(
                accumulator.pairs(),
                options.thresholds,
                options.hard_limit);
            ...
            if (!options.hard_cache_index_output.empty()) {
                write_report(
                    pfm::cache_eval::hardCacheIndexCsv(
