...
struct Modules {
    pfm::Backbone backbone{nullptr};
    pfm::SparseHead sparse_head{nullptr};
    pfm::DenseHead dense_head{nullptr};
    pfm::PlanetaryGraphMatcher graph_matcher{nullptr};
};
...
Options parseOptions(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (arg == "--base") {
            ...
            std::cout
                << "Usage: pfm_interpolate_checkpoints --base stable.pt --candidate tuned.pt "
...
CheckpointConfig readConfig(const std::string& checkpoint) {
    ...
    return CheckpointConfig{
        readConfigInt64(config_archive, "base_channels"),
        readConfigInt64(config_archive, "descriptor_dim"),
        readConfigInt64(config_archive, "graph_hidden_dim"),
        readConfigInt64(config_archive, "graph_attention_layers")};
}
...
Modules makeModules(const CheckpointConfig& config) {
    Modules modules;
    modules.backbone = pfm::Backbone(INPUT_CHANNELS, config.base_channels);
    modules.sparse_head = pfm::SparseHead(
        config.base_channels * SPARSE_FEATURE_CHANNEL_MULTIPLIER,
        config.descriptor_dim);
    modules.dense_head = pfm::DenseHead(config.base_channels);
    modules.graph_matcher = pfm::PlanetaryGraphMatcher(
        config.descriptor_dim,
...
void interpolateNamedTensors(
    const torch::OrderedDict<std::string, torch::Tensor>& base,
    const torch::OrderedDict<std::string, torch::Tensor>& candidate,
    double alpha
) {
...
int main(int argc, char** argv) {
    try {
        ...
        std::cout << "interpolated checkpoint: output=" << options.output_checkpoint
                  << " alpha=" << options.alpha << '\n';
        return 0;
